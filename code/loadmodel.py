...
def load_restore(
	...
	modelfactory = test_fashion.ModelFactory(recipe, ablation_type = ablation_type)
	model = modelfactory.makeModel(device)
...
def load_model(
	...
	if loaded:
		...
		model, optim  = load_restore(device, loaded, recipe, lr, ablation_type = ablation_type)
