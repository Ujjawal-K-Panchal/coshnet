...
class DecTTLinear(nn.Module):
    def __init__(self, in_features, out_features, source_weight, source_bias, shape=None,
                 d=3, tt_rank=8, epsilon = 1e-4, auto_shape_mode='ascending',
                 auto_shape_criterion='entropy', forward_mode=None,
                 ):
        ...
        init = t3.decompositions.to_tt_matrix(source_weight,
                                            shape = shape,
                                            max_tt_rank = tt_rank,
                                            epsilon = epsilon
                                            )
        ...
        if source_bias is not None:
            self.bias = source_bias.clone().detach().requires_grad_(True)
        else:
